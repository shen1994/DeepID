...
def folder_for_crop(db_folder, result_folder, crop_size):
    ...
    for people_folder in os.listdir(db_folder):
        number += 1
        src_people_path = db_folder + os.sep + people_folder + os.sep
        des_people_path = result_folder  + os.sep + people_folder + os.sep
        ...
        for vedio_folder in os.listdir(src_people_path):
            step += 1
            src_vedio_path = src_people_path + vedio_folder + os.sep
            des_vedio_path = des_people_path + vedio_folder + os.sep
            if not os.path.exists(des_vedio_path):
                os.mkdir(des_vedio_path)
            for img_file in os.listdir(src_vedio_path):
                counter += 1
                src_img_path = src_vedio_path + img_file
                des_img_path = des_vedio_path + img_file
